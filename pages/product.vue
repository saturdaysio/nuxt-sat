        ...
        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
          <DialogPanel class="relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl">
            <div class="flex px-4 pb-2 pt-5">
              <Button buttton-type="button" button-label="Sign in" button-class="-m-2 inline-flex items-center justify-center rounded-md p-2 text-gray-400" @click="open = false">
              ...
              <div class="border-b border-gray-200">
                <TabList class="-mb-px flex space-x-8 px-4">
                  <Tab as="template" v-for="category in navigation.categories" :key="category.name" v-slot="{ selected }">
                    <Button buttton-type="button" :button-label="category.name" :button-class="`${selected ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-300'} flex-1 whitespace-nowrap border-b-2 px-1 py-4 text-base font-medium`" />
                  </Tab>
                </TabList>
              </div>
                ...
                <TabPanel v-for="category in navigation.categories" :key="category.name" class="space-y-10 px-4 pb-8 pt-10">
                    ...
                    <div v-for="(item, itemIdx) in category.featured" :key="itemIdx" class="group aspect-h-1 aspect-w-1 relative overflow-hidden rounded-md bg-gray-100">
                      ...
                      <div class="flex flex-col justify-end">
                        <div class="bg-white bg-opacity-60 p-4 text-base sm:text-sm">
                          <a :href="item.href" class="font-medium text-gray-300">
                  ...
                  <div v-for="(column, columnIdx) in category.sections" :key="columnIdx" class="space-y-10">
                    <div v-for="section in column" :key="section.name">
                      <p :id="`${category.id}-${section.id}-heading-mobile`" class="font-medium text-gray-300">{{ section.name }}</p>
...
<div class="space-y-6 border-t border-gray-200 px-4 py-6">
              <div v-for="page in navigation.pages" :key="page.name" class="flow-root">
                <a :href="page.href" class="-m-2 block p-2 font-medium text-gray-300">{{ page.name }}</a>
              </div>
            </div>
...
<div class="border-t border-gray-200 px-4 py-6">
              <a href="#" class="-m-2 flex items-center p-2">
                ...
                <span class="ml-3 block text-base font-medium text-gray-300">CAD</span>
...
<header class="relative bg-gray-900/10">
    <nav aria-label="Top" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="border-b border-white/25">
        <div class="flex h-16 items-center justify-between">
          <div class="flex flex-1 items-center lg:hidden">
            <Button buttton-type="button" button-label="Open menu" button-class="-ml-2 rounded-md bg-white p-2 text-gray-400" @click="open = true">
                      ...
                      <div class="mx-auto max-w-7xl px-8">
                        <div class="grid grid-cols-2 gap-x-8 gap-y-10 py-16">
                          ...
                          <div class="grid grid-cols-3 gap-x-8 gap-y-10 text-sm text-gray-500">
                            <div v-for="(column, columnIdx) in category.sections" :key="columnIdx" class="space-y-10">
                              <div v-for="section in column" :key="section.name">
                                <p :id="`${category.id}-${section.id}-heading`" class="font-medium text-gray-300">{{ section.name }}</p>
...
<main class="mx-auto mt-8 max-w-2xl px-4 pb-16 sm:px-6 sm:pb-24 lg:max-w-7xl lg:px-8">
    <div class="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
      ...
      <div class="mt-8 lg:col-span-5">
        <form>
          <!-- Color picker -->
          <div>
            <h2 class="text-md font-bold text-gray-300">Color</h2>
          ...
          <div class="mt-8">
            <div class="flex items-center justify-between">
              <h2 class="text-md font-bold text-white">Size</h2>
              <a href="#" class="text-sm font-bold text-blue-600 hover:text-indigo-500">See sizing chart</a>
            </div>
            ...
            <RadioGroup v-model="selectedSize" class="mt-2">
              ...
              <div class="grid grid-cols-3 gap-3 sm:grid-cols-6">
                <RadioGroupOption as="template" v-for="size in product.sizes" :key="size.name" :value="size" :disabled="!size.inStock" v-slot="{ active, checked }">
                  <div :class="[size.inStock ? 'cursor-pointer focus:outline-none' : 'cursor-not-allowed opacity-25', active ? 'ring-2 ring-indigo-500 ring-offset-2' : '', checked ? 'border-transparent bg-indigo-600 text-white hover:bg-indigo-700' : 'border-gray-200 bg-white text-gray-300 hover:bg-gray-50', 'flex items-center justify-center rounded-md border py-3 px-3 text-sm font-medium uppercase sm:flex-1']">
          ...
          <Button buttton-type="submit" button-label="Add to cart" button-class="mt-8 flex w-full items-center justify-center rounded-md border border-transparent bg-green-600 px-8 py-3 text-base font-bold text-black hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"/>

        </form>

        <!-- Product details -->
        <div class="mt-10">
          <h2 class="text-xl font-bold text-gray-300">Description</h2>

          <div class="prose prose-sm mt-4 text-gray-300" v-html="product.description" />
        </div>

        <div class="mt-8 border-t border-white/25 pt-8">
          <h2 class="text-xl font-bold text-gray-300">Fabric &amp; Care</h2>

          <div class="prose prose-sm mt-4 text-gray-300">
            <ul role="list">
          ...
          <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <div v-for="policy in policies" :key="policy.name" class="rounded-lg border border-gray-200 bg-gray-50 p-6 text-center">
              <dt>
                ...
                <span class="mt-4 text-base font-bold text-white">{{ policy.name }}</span>
              </dt>
              <dd class="mt-1 text-md text-gray-400">{{ policy.description }}</dd>
            </div>
          </dl>
        </section>
      </div>
    ...
    <section aria-labelledby="reviews-heading" class="mt-16 sm:mt-24">
      <h2 id="reviews-heading" class="text-lg font-medium text-gray-300">Recent reviews</h2>
        ...
        <div v-for="review in reviews.featured" :key="review.id" class="pt-10 lg:grid lg:grid-cols-12 lg:gap-x-8">
          <div class="lg:col-span-8 lg:col-start-5 xl:col-span-9 xl:col-start-4 xl:grid xl:grid-cols-3 xl:items-start xl:gap-x-8">
            ...
            <div class="mt-4 lg:mt-6 xl:col-span-2 xl:mt-0">
              <h3 class="text-sm font-medium text-gray-300">{{ review.title }}</h3>
          ...
          <div class="mt-6 flex items-center text-sm lg:col-span-4 lg:col-start-1 lg:row-start-1 lg:mt-0 lg:flex-col lg:items-start xl:col-span-3">
            <p class="font-medium text-gray-300">{{ review.author }}</p>
    ...
    <section aria-labelledby="related-heading" class="mt-16 sm:mt-24">
      <h2 id="related-heading" class="text-lg font-medium text-gray-300">Customers also purchased</h2>
      ...
      <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
        <div v-for="relatedProduct in relatedProducts" :key="relatedProduct.id" class="group relative">
          ...
          <div class="mt-4 flex justify-between">
            ...
            <p class="text-sm font-medium text-gray-300">{{ relatedProduct.price }}</p>
          </div>
        </div>
      </div>
    </section>
...
useHead({
  title: 'Saturdays.io - Product page',
  meta: [
